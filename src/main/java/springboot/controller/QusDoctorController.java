...
public class QusDoctorController {
	...
	public String submitAddInfo(HttpServletRequest request) {// ,@RequestParam() ,@RequestParam("picture")MultipartFile
																// fileUpload
		// d_name role sex email phone cDate province city area time room1 room2
		// description
		...
		System.out.println("uName \t" + uName + " role:" + role + "\t sex:\t" + sex + "phone:" + phone + "province\t"
				+ province + "city:\t" + city + "\t" + "area:\t" + area + "time:\t" + time + "room1:\t" + room1
				+ "\t room2:" + room2 + "description:\t" + description);
		/*
		 * //获取文件名 String fileName = fileUpload.getOriginalFilename();
		 * System.out.println(fileName); //获取文件后缀名 String suffixName =
		 * fileName.substring(fileName.lastIndexOf(".")); //重新生成文件名 fileName =
		 * UUID.randomUUID()+suffixName; //指定本地文件夹存储图片 String filePath =
		 * "C:/Users/zcx/Desktop/毕业项目/test/QusProject/src/main/resources/static/mypicture/";
		 * try {// //将图片保存到static文件夹里 System.out.println(filePath+fileName);
		 * fileUpload.transferTo(new File(filePath+fileName)); } catch (Exception e) {
		 * e.printStackTrace(); return "back/page/doctor/Doctor.html"; }
		 */
	...
	@RequestMapping(value = "upload/uploadPic.do", method = RequestMethod.POST) // @RequestParam("picture")MultipartFile
																				// picture,
	public JSONObject uploadPic(@RequestParam(value = "file", required = false) MultipartFile file,
		...
		if (file != null) {
			...
			/*
			 * prefix=originalName.substring(originalName.lastIndexOf(".")+1); dateStr =
			 * format.format(new Date()); String filepath =
			 * request.getServletContext().getRealPath("/static") + uploadDir + dateStr +
			 * "." + prefix; filepath = filepath.replace("\\", "/"); File files=new
			 * File(filepath); //打印查看上传路径 System.out.println(filepath);
			 * if(!files.getParentFile().exists()){ files.getParentFile().mkdirs(); }
